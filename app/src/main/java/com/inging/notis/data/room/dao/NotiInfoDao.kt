...
interface NotiInfoDao : BaseDao<NotiInfo> {
    ...
    // 최신 노티 (패키지)
    @Query("select * from NotiInfo where pkgName = :pkgName order by timestamp desc limit 1")
    fun getRecentNotiInfoByPkgName(pkgName: String): NotiInfo?
    ...
    // 노티 목록 (패키지 별)
    @Query("select * from NotiInfo where pkgName = :pkgName order by timestamp desc")
    fun getNotiInfoListByPkgName(pkgName: String): PagingSource<Int, NotiInfo>
    ...
    // 노티 갯수 (패키지 / 서머리 텍스트 별)
    @Query("select count(*) from NotiInfo where pkgName = :pkgName and summaryText = :summaryText")
    ...
    // 노티 갯수 (패키지 / 메시지 아닌)
    @Query("select count(*) from NotiInfo where pkgName = :pkgName and category != 'msg'")
