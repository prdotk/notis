...
class BlockAppsViewHolder(
    ...
    fun bind(
        ...
        binding.run {
            ...
            onOff.isChecked = info.isBlock
            onOff.setOnCheckedChangeListener { _, isChecked ->
                info.isBlock = isChecked
    ...
    companion object {
        fun getInstance(parent: ViewGroup): BlockAppsViewHolder {
            val binding = LayoutAppListItemBinding.inflate(
                LayoutInflater.from(parent.context), parent, false
            )
            return BlockAppsViewHolder(binding)
        }
    }
}