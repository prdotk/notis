...
class BlockAppsFragment : Fragment() {
    ...
    private val _adapter: BlockAppsAdapter by lazy {
        BlockAppsAdapter(allOnOff) { mode, info ->
            when (mode) {
                ClickMode.DEFAULT -> {
    ...
    override fun onCreateView(
        ...
        binding.run {
            title.text = getText(R.string.more_block_notifications)
...
//            itemAnimator = null
                adapter = _adapter.apply {
                    lifecycleScope.launch {
                        ...
                        viewModel.getBlockAppList(requireContext())
                        submitList(
                            if (viewModel.showSystemApp) {
    ...
    override fun onAttach(context: Context) {
        ...
        callback = object : OnBackPressedCallback(true) {
            ...
            requireActivity().onBackPressedDispatcher.addCallback(this@BlockAppsFragment, this)
        }
    }
    ...
    private fun allOn() {
        AlertDialog.Builder(requireContext())
            ...
            .setPositiveButton(R.string.dialog_ok) { _, _ ->
                viewModel.appInfoList.forEach { info -> info.isBlock = true }
                allOnOff.set(1)
                viewModel.allBlockSet(true)
    ...
    private fun allOff() {
        AlertDialog.Builder(requireContext())
            ...
            .setPositiveButton(R.string.dialog_ok) { _, _ ->
                viewModel.appInfoList.forEach { info -> info.isBlock = false }
                allOnOff.set(0)
                viewModel.allBlockSet(false)
            }.setNegativeButton(R.string.dialog_cancel) { _, _ ->
            }.create().show()
    }
}